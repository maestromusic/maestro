...
#
# Module for tag handling (initalizing tag lists, convert tag-IDs to tagnames and vice versa, etc.).
# Call updateIndexedTags at program start to initialize and use either the get-method or the constants (TITLE, ARTIST, etc.) to create tags.
...
# Tags which have a special meaning for the application and cannot always be treated generically. This allows to use tags.TITLE instead of tags.get(config.get("tags","title_tag")) as the user may decide to use another tagname than "title" for his titles.
...
class Tag:
    ...
    def isIndexed(self):
        """Returns whether this tag is indexed (i.e. of type IndexedTag)."""
...
def get(identifier):
    """Returns the tag identified by <identifier>. If <identifier> is an integer this method returns the indexed tag with this id. If <identifier> is a string this method returns the tag with this name (of type IndexedTag if there is an indexed tag of this name, otherwise OtherTag). In the case of indexed tags this method does not create new instances of the tags but returns always the same instance."""
...
def updateIndexedTags():
    """Initializes (or updates) the variables of this module based on the information of the tagids-table. At program start or after changes of that table this method must be called to ensure the module has the correct indexed tags and their IDs."""
