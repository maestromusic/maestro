...
class BrowserModel(rootedtreemodel.RootedTreeModel):
    ...
    def __init__(self,layers,sortTags):
        super().__init__(levels.real, RootNode(self))
        self.table = None
        self.layers = layers
    ...
    def _loadContainerLayer(self,node,table):
        ...
        while len(childIds) > 0:
            ...
            childIds = list(itertools.chain.from_iterable(levels.real.get(id).contents.ids for id in childIds if levels.real.get(id).isContainer()))        
