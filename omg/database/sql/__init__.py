...
* 'qtsql': uses the QtSql-Module of PyQt (http://doc.trolltech.com/latest/qtsql.html)
...
The main class of the module is AbstractSQL which is subclassed by
every driver.
"""
...
def newConnection(drivers):
    ...
    DBException is raised. This method does not actually open a connection, but only sets up the the
    connection object.
    """
    for driver in drivers:
        try:
            if driver not in _modules:
                import importlib
                _modules[driver] = importlib.import_module("."+driver, __package__)
...
class AbstractSql:
    ...
    newConnection which will create a non-abstract subclass of this depending on the driver.
    """
    def __init__(self):
    ...
    def query(self, queryString, *args, **kwargs):
        ...
        Execute the query *queryString* and return an AbstractSqlResult which yields the result's
        rows in tuples. Arguments and keyword-arguments can be used to insert data and variable parts into
        the query:
        *queryString* may contain '?' as placeholders which are replaced by the *args*-parameters in the
        given order. If possible the arguments are sent to the database engine without being transformed
        to strings and parsed back which makes this the preferred way to pass data into queries.
        A consequence is that you must not use quotation marks around string
        parameters:
        ...
        will not work.
        Keyword-arguments, on the other hand, are replaced directly into the string using the builtin
        'format' function. Thus above example can be written as
        
            SELECT id FROM {table}
            
        One keyword is always defined: '{p}' will be replaced by the database prefix.
        """
        
    def multiQuery(self, queryString, args, **kwargs):
        ...
        In some drivers multiQuery may be faster than this loop because the query has to be sent to
        ...
        self.transaction()
        for parameters in args:
            self.query(queryString, *parameters, **kwargs)
        self.commit()
...
class AbstractSqlResult:
    ...
    rows. AbstractSqlResult-subclasses implement __iter__ so they may be used in for-loops
    ...
    def next(self):
        """Yields the next row from the result set or raises a StopIteration if there is no such
        row."""
