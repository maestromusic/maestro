...
class MPDClient(object):
    ...
    def _send(self, command, args):
        if self._command_list is not None:
            raise CommandListError("Cannot use send_{} in a command list".format(
                                   command.replace(" ", "_")))
    ...
    def _fetch(self, command, args=None):
        if self._command_list is not None:
            raise CommandListError("Cannot use fetch_{} in a command list".format(
                                   command.replace(" ", "_")))
        if self._iterating:
            raise IteratingError("Cannot use fetch_{} while iterating".format(
                                 command.replace(" ", "_")))
        if not self._pending:
            raise PendingCommandError("No pending commands to fetch")
        if self._pending[0] != command:
            raise PendingCommandError("'{}' is not the currently "
                                      "pending command".format(command))
        del self._pending[0]
        retval = self._commands[command]
        if callable(retval):
            return retval()
    ...
    def _execute(self, command, args):
        if self._iterating:
            raise IteratingError("Cannot execute '{}' while iterating".format(
                                 command))
        if self._pending:
            raise PendingCommandError("Cannot execute '{}' with "
                                      "pending commands".format(command))
        retval = self._commands[command]
        if self._command_list is not None:
            if not callable(retval):
                raise CommandListError("'{}' not allowed in command list".format(
                                        command))
            self._write_command(command, args)
    ...
    def _read_list(self):
        ...
        for key, value in self._read_pairs():
            if key != seen:
                if seen is not None:
                    raise ProtocolError("Expected key '{}', got '{}'".format(
                                        (seen, key)))
                seen = key
            yield value
